...
Mencari pola hubungan antar data dalam konteks metode \textit{fuzzy time series} melibatkan analisis deret waktu untuk mengidentifikasi tren dan pola dalam data historis. Metode ini menggunakan himpunan \textit{fuzzy} untuk menangkap sifat ketidakpastian dan variabilitas dalam data deret waktu. Pertama, data historis diubah menjadi himpunan \textit{fuzzy} berdasarkan interval yang telah ditentukan. Kemudian, hubungan antar himpunan \textit{fuzzy} dianalisis untuk menemukan pola perubahan dan tren dari satu periode ke periode berikutnya.

Dalam pendekatan \textit{fuzzy time series}, aturan \textit{fuzzy} dibuat untuk menggambarkan hubungan antar himpunan \textit{fuzzy}. Aturan ini dibentuk berdasarkan pola yang ditemukan dalam data historis, misalnya bagaimana suatu nilai \textit{fuzzy} pada periode tertentu mempengaruhi nilai \textit{fuzzy} pada periode berikutnya. Proses ini memungkinkan kita untuk membangun model prediksi yang mampu menangkap dinamika temporal dari data. Dengan demikian, metode \textit{fuzzy time series} tidak hanya membantu dalam memahami pola hubungan antar data, tetapi juga menyediakan kerangka kerja yang efektif untuk membuat prediksi dan analisis tren jangka panjang dalam situasi yang penuh ketidakpastian.
...
Untuk menentukan interval data dan jumlah himpunan \textit{fuzzy}, digunakan rumus Sturges. Berdasarkan rumus tersebut, data dibagi menjadi beberapa interval yang digunakan untuk menentukan jumlah himpunan \textit{fuzzy}. Menggunakan rumus Sturges, didapatkan jumlah himpunan \textit{fuzzy} sebanyak 6 interval data. Interval data ini kemudian digunakan sebagai dasar untuk membangun himpunan \textit{fuzzy} dan hubungan logika \textit{fuzzy}.
...
\subsection{Mendefinisikan \textit{fuzzy sets} berdasarkan interval data dan data historis}
Setiap interval kemudian diwakili oleh himpunan \textit{fuzzy}. Dalam penelitian ini, digunakan 6 interval data yang masing-masing diwakili oleh himpunan \textit{fuzzy}, yaitu:
\begin{figure}[H]
    \centering
...
\subsection{Membangun hubungan logika \textit{fuzzy}}
Langkah berikutnya adalah membangun hubungan logika \textit{fuzzy} untuk menggambarkan hubungan antar himpunan \textit{fuzzy} dari data historis harga singkong. Dengan menggunakan metode Fuzzy Logical Relationship (FLR), didapatkan hubungan logika \textit{fuzzy} sebagai berikut:
\begin{figure}[H]
    \centering
...
\subsection{Mencari pola hubungan antar data}
Langkah selanjutnya adalah mencari pola hubungan antar data pada data yang telah diperoleh. Dengan menggunakan metode \textit{fuzzy time series} (FTS), didapatkan pola hubungan antar data sebagai berikut:
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{images/Pola data.png} 
    \caption{Pola Hubungan Antar Data}
\end{figure}
Data di atas merupakan hasil dari pencarian pola hubungan antar data yang dilakukan dengan menggunakan metode FTS. Data ini diambil sebagian saja sebagai contoh visualisasi pola hubungan antar data.

\subsection{Memasukkan data input ke dalam model FTS dan melakukan peramalan}
Berikut adalah contoh hasil peramalan harga singkong menggunakan model FTS:
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{images/Peramalan.png} 
...
\bibitem{b16} Author16, A. (Year). Title of the Sixteenth Reference. Journal Name, Volume(Issue), Page-Page. DOI: XXXX

\bibitem{b17} Author17, A. (Year). Title of the Seventeenth Reference. Journal Name, Volume(Issue), Page-Page. DOI: XXXX

\bibitem{b18} Author18, A. (Year). Title of the Eighteenth Reference. Journal Name, Volume(Issue), Page-Page. DOI: XXXX

\bibitem{b19} Author19, A. (Year). Title of the Nineteenth Reference. Journal Name, Volume(Issue), Page-Page. DOI: XXXX

\bibitem{b20} Author20, A. (Year). Title of the Twentieth Reference. Journal Name, Volume(Issue), Page-Page. DOI: XXXX
